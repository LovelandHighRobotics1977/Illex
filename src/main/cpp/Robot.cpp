...
void Robot::RobotInit() {
  ...
  double kI = 0;
  double kD = 0;
  double kF = 0; //Feed Forward
  double gRef = 0; //Setpoint 8*/
  ...
  m_l1.ConfigStatorCurrentLimit(StatorCurrentLimitConfiguration(true,40,45,1),100);
  m_l2.ConfigStatorCurrentLimit(StatorCurrentLimitConfiguration(true,40,45,1),100);
  m_r1.ConfigStatorCurrentLimit(StatorCurrentLimitConfiguration(true,40,45,1),100);
  m_r2.ConfigStatorCurrentLimit(StatorCurrentLimitConfiguration(true,40,45,1),100);
  
  m_shooter.ConfigFactoryDefault();
  m_shooter.ConfigSelectedFeedbackSensor(FeedbackDevice::IntegratedSensor, 0, 0); // PIDLoop=0, timeoutMs=0

  m_shooter.Config_kP(0, 0.05);
  m_shooter.Config_kI(0, 0);
  m_shooter.Config_kD(0, 0);
  m_shooter.Config_kF(0, 1);

  m_shooter.ConfigNominalOutputForward(0);
...
void Robot::AutonomousPeriodic() {
  ...
  if(fired == false){
    getDistance();
    if(((distance < 85)||(distance > 90))&&(stage1 == true)){
      rangeFind(85,90);
    }else{
      stage1 = false;
      getDistance();
      if(((distance < 80)||(distance > 85))&&(stage2 == false)){
        AutoShoot(0,0,0);
        stack(0,0);
        getDistance();
        rangeFind(80,85);
...
void Robot::TeleopPeriodic() {
  ...
  climberArm(m_driverController.GetXButton(), m_driverController.GetBButton());
  /*}else{
  ...
  highGoal(75,80,m_driverController.GetRightBumper());
...
void Robot::shoot(float speed1, float speed2, int reverse){
  /*if(speed1 > .1){
    ...
    //m_shooter.Set(TalonFXControlMode.Velocity, 100);
    m_shooter.Set(TalonFXControlMode::Velocity, 925);
  }else{
    m_shooter.Set(TalonFXControlMode::Velocity, 0);
  }/*else if(reverse == 1){
    //m_shooter.Set(-1);
  }else{
    //m_shooter.Set(0);
...
void Robot::AutoShoot(float speed1, float speed2, int reverse){
  /*if(speed1 > .1){
    m_shooter.Set(speed1);
    //m_shooter.Set(.3);
  }else*/ if(speed2 > .475){
    m_shooter.Set(TalonFXControlMode::Velocity, 965);
  }else{
    m_shooter.Set(TalonFXControlMode::Velocity, 0);
  }
...
void Robot::getDistance(){
  ...
  distance = (((HeightToTarget - HeightToCamera)/tan(goalAngleRAD)) - DistanceToBumper);
  std::cout<<"Distance: "<<distance<<" angle: "<<goalAngleRAD<<std::endl;
}
...
void Robot::climberArm(int in, int out){
  if(in == true){
    arm1.Set(true);
    arm2.Set(false);
  }else if(out == true){
...
void Robot::highGoal(float less, float more, int status){
  if(status == 1){
  getDistance();
    if((distance < less)||(distance > more)){
      getDistance();
