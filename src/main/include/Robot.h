...
class Robot : public frc::TimedRobot {
 public:
  ...
  void climberArm(int in, int out);
  ...
  void AutoShoot(float speed1, float speed2, int reverse);

  void highGoal(float less, float more, int status);
  ...
  WPI_TalonFX m_arm{18};
  WPI_TalonFX m_shooter{15};
  ...
  frc::Solenoid arm1{frc::PneumaticsModuleType::CTREPCM, 2};
  frc::Solenoid arm2{frc::PneumaticsModuleType::CTREPCM, 3};
  ...
  double HeightToCamera = 41;
  double CameraAngle = 40;
  double DistanceToBumper = 29;
  double distance;
  double goalAngleDEG;
  double goalAngleRAD;
